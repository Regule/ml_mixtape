...
def arg_column_list(txt: str)-> Tuple[str,...]:
    ...
    if not column_names:
        raise argparse.ArgumentTypeError('Expected a nonepty, comma separated, list.')
...
def arg_positive_int(txt: str)-> int:
    try:
        val = int(txt)
        if not val > 0:
            raise argparse.ArgumentTypeError('Argument value must be greater than 0')
        return val
    except ValueError:
        raise argparse.ArgumentTypeError(f'Unable to convert "{txt}" to valid integer')
...
class Config:
    ...
    def from_args(argv: Optional[Sequence[str]] = None)-> Config:
        ...
        # Declare arguments
        prsr.add_argument('--train_csv', required=True, type=str, help='Path to training CSV')
        prsr.add_argument('--test_csv', required=True, type=str, help='Path to test CSV')
